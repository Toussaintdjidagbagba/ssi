...
class HistoriqueController extends Controller 
{
    public function gethist(){
        ...
        $hists = DB::table('historiques')->orderby('id', 'DESC')->get();
    ...
    public function getHistClientAuAdmin(){
        ...
        if(request('rec') == 1){
            if(request('check') != "" && request('check') != null){
                ...
                if(isset($id_usr->id))
                {
                    $hists = $hists->where('user_action', $id_usr->id)->orderby('historique_clients.id', 'DESC')->get();
                }
                else
                    $hists = $hists->where('user_action', "")->orderby('historique_clients.id', 'DESC')->get();
                return view("admin.historiqueTranslationClient", compact('hists', 'search'));
            }else{
                $hists = $hists->orderby('historique_clients.id', 'DESC')->get();
                return view("admin.historiqueTranslationClient", compact('hists', 'search'));
            }
        }
