...
class DemandeClientController extends Controller 
{
    ...
    /**
    *
    *  setnsiagbodjekwo - Produit de NSIA 
    */
    public function setnsiagbodjekwo(Request $request)
    {
        if (!auth()->guest()) {
            ...
            // Verification du compte avoirs du client en espèce
                    if(isset(DB::table('avoirs')->where('id_user', auth()->user()->id)->where('gainvirtuel', '>=', $valeuradhesion)->get()[0]->gainvirtuel))
                    {
                        if ($request->hasFile('piece')) {
                            $referenceNote = "nsiag-".date('ymdhis');
                            $namefile = $referenceNote.'.'.$request->file('piece')->getClientOriginalExtension();
                            $upload = "nsia/upload/";
                            ...
                            //update la table
                            DB::table('systemadmins')->update([
                                    'compteavoirrecu' => $recu
                                    ]);
                            $descript = "Numéro pièce d'identité : ".request('numcin')." | Nom père : ".request('nomp')." | Date de naissance père : ".request('naisp')." | Profession père : ".request('profp'). " | ".
                            " | Nom mère : ".request('nomm')." | Date de naissance mère : ".request('naism')." | Profession mère : ".request('profm'). ".";
                            
                            $add = new NSIA();
                            $add->idUser = auth()->user()->id;
                            $add->numpiece = request('numcin');
                            $add->naispere = request('naisp');
                            $add->profpere = request('profp');
                            $add->nompere = request('nomp');
                            $add->naismere = request('naism');
                            $add->profmere = request('profm');
                            $add->nommere = request('nomm');
                            $add->montant = $valeuradhesion;
                            $add->libelle = "NSIAG";
                            $add->num = request('num');
                            $add->doc = $path;
                            $add->save();   
                            ...
                            $add->NomUser = auth()->user()->nom;
                            $add->EmailUser = auth()->user()->email;
                            $add->CodePersoUser = auth()->user()->codeperso;
                            $add->Tel = request('num');
                            $add->MontantPayer = $valeuradhesion;
                            $add->libelle = "NSIAG";
                            $add->fichie = $path;
                            $add->save();*/
                            
                            //flash("Vous recevez un message de confirmation dans un instant.");
                            $message = "Vous ".auth()->user()->nom." (".auth()->user()->email.") avez payé ".$valeuradhesion." SSI pour souscrire à NSIA GBODJEKWE avec le numéro ".request('num').".";
                            HistoriqueClient::saveHistorique($message, auth()->user()->id );
                            flash($message);
                            flash("Votre demande est en attente de traitement.. Télécharger l'application NSIA Vie sur playStore et connectez vous sous peu.");
                            return Back();
                    
                        
                        }else{
                            flash("Veuillez importer une image de votre pièce d'identité. ");
                            return Back();
                        }   
                    }
                    else
		...
		{
		   flash("Vous devez être connecté pour effectuée l'achat de SSI. Si vous ne possédez pas de compte, inscrivez-vous et revenez sur NSIA GBODJEKWE");
        ...
        if (!auth()->guest()) {
            ...
            if($resultat){
                ...
                if(isset(DB::table('avoirs')->where('id_user', auth()->user()->id)->where('gainvirtuel', '>=', $valeuradhesion)->get()[0]->gainvirtuel))
                {
                    ...
                    $add->FraisSSI = $fraisajouter;
                    $add->save();
                            
                    $message = "Vous avez demandé un dépôt sur carte visa de ".request('montant')." SSI sur votre ".request('intituler')."
    ...
    */
    ...
    public function setsbee1(Request $request)
    {
        request()->validate([
            ...
            'prenomm' => 'required|string',
            'policem' => 'required|string',
            'numWham' => 'required|min:0',
            'mailm' => 'required|string',
            'montantm' => 'required',
            ]);
            
        $valeuradhesionm = request('montantm');
        ...
        if (!auth()->guest()) {
            ...
            if(isset(DB::table('avoirs')->where('id_user', auth()->user()->id)->where('gainvirtuel', '>=', $valeuradhesion)->get()[0]->gainvirtuel))
            {
                ...
                Sbeecarte::create([
                    ...
                    'Email' => request('mailm'), 
                    'WhatsApp' => request('numWham'), 
                    'Police' => request('policem'), 
                    'Montant' => $valeuradhesionm,
                    'MontantPayer' => $valeuradhesion,
                    'FraisSSI' => $fraisajouter,
                    'RefRecu' => $refid, 
    ...
    public function setconfirmemtn(Request $request){
        $num = request('numero');
        $forfait = request('forfait');
        return view('services_site.mtn.confirm', compact('num', 'forfait'));
    }
    ...
    public function setmtn(Request $request)
    {
        ...
        if (InterfaceServiceProvider::verifie_mtn($id_num) == 1) {
            ...
            if (request('lib') == "internet") {
                ...
                switch (request('forfaitlib')) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                        $forfait = "FORFAIT INTERNET MTN : 1.99 SSI (72h)";
                        $valeuradhesion = 1.99;
                        break;
                    case 3:
                    ...
                    case 10:
                        ...
                        break;
                    case 14:
                        $forfait = "FORFAIT INTERNET MTN : 40 SSI (illimité)";
                        $valeuradhesion = 40;
                        break;
                    case 15:
                        $forfait = "FORFAIT INTERNET MTN : 50 SSI (illimité)";
                        $valeuradhesion = 50;
                        break;
                    case 16:
                        $forfait = "FORFAIT INTERNET MTN : 60 SSI (illimité)";
                        $valeuradhesion = 60;
                        break;
                    case 17:
                        $forfait = "FORFAIT INTERNET MTN : 100 SSI (illimité)";
                        $valeuradhesion = 100;
                        break;
                    case 18:
                        $forfait = "FORFAIT INTERNET MTN : 150 SSI (illimité)";
                        $valeuradhesion = 150;
                        break;
                    default:
                        $forfait = "inconnue";
            ...
            if (request('lib') == "depot") {
                ...
                $valmotant = request('forfait');
                
                $fraisajouter = $valmotant * InterfaceServiceProvider::fraisssidepot() / 100 ;

                $valeuradhesion = $valmotant + $fraisajouter;
                ...
                if (!auth()->guest()) {
                    ...
                    if(isset(DB::table('avoirs')->where('id_user', auth()->user()->id)->where('gainvirtuel', '>=', $valeuradhesion)->get()[0]->gainvirtuel))
                    {
                        ...
                        //flash("Vous recevez un message de confirmation dans un instant.");
                        $message = "Vous ".auth()->user()->nom." (".auth()->user()->email.") avez payé ".$valmotant." 
                        SSI pour ".$forfait." sur le numéro ".request('numero').".  Frais SSI : ".$fraisajouter." SSI soit 0.5% du montant";
                        HistoriqueClient::saveHistorique($message, auth()->user()->id );
                        flash($message);
                        return Back();
                    }
    ...
    /**
     * getceltiis - Achat de crédit celtiis
     * 
     */
    public function getceltiis()
    {
        return view('services_site.celtiis.celtiis');
    }
    
    public function setconfirmeceltiis(Request $request){
        $num = request('numero');
        $forfait = request('forfait');
        return view('services_site.mtn.confirm', compact('num', 'forfait'));
    }

    public function setceltiis(Request $request)
    {
        $num = substr(request('numero'), -8);
        ...
        if (InterfaceServiceProvider::verifie_celtiis($id_num) == 1) {
            ...
            if (request('lib') == "credit") {
                ...
                {
                    flash("Vous devez être connecté pour effectuée l'achat et payer les frais à partir de vos compte avoirs. Si vous ne possédez pas de compte, inscrivez-vous et revenez sur CELTIIS")->error();
                    return redirect('/connexion');
                }
            ...
            if (request('lib') == "forfaitappel") {
                ...
                switch (request('forfaitlib')) {
                    case 1:
                        $forfait = "FORFAIT APPEL CELTIIS : 0.4 SSI (24h)";
                        $valeuradhesion = 0.4;
                        break;
                    case 2:
                        $forfait = "FORFAIT APPEL CELTIIS : 0.6 SSI (24h)";
                        $valeuradhesion = 0.6;
                        break;
                    case 3:
                        $forfait = "FORFAIT APPEL CELTIIS : 1 SSI (07jrs)";
                        $valeuradhesion = 1;
                        break;
                    case 4:
                        $forfait = "FORFAIT APPEL CELTIIS : 2 SSI (07jrs)";
                        $valeuradhesion = 2;
                        break;
                    case 5:
                        $forfait = "FORFAIT APPEL CELTIIS : 3 SSI (07jrs)";
                        $valeuradhesion = 3;
                        break;
                    case 6:
                        $forfait = "FORFAIT APPEL CELTIIS : 4 SSI (07jrs)";
                        $valeuradhesion = 4;
                        break;
                    case 7:
                        $forfait = "FORFAIT APPEL CELTIIS : 6 SSI (30jrs)";
                        $valeuradhesion = 6;
                        break;
                    case 8:
                        $forfait = "FORFAIT APPEL CELTIIS : 10 SSI (30jrs)";
                        $valeuradhesion = 10;
                        break;
                    case 9:
                        $forfait = "FORFAIT APPEL CELTIIS : 14 SSI (30jrs)";
                        $valeuradhesion = 14;
                        break;
                    case 10:
                        $forfait = "FORFAIT APPEL CELTIIS : 18 SSI (30jrs)";
                        $valeuradhesion = 18;
                        break;
            ...
            if (request('lib') == "internet") {
                ...
                switch (request('forfaitlib')) {
                    ...
                    case 3:
                        $forfait = "FORFAIT INTERNET MTN : 1.5 SSI (72h)";
                        $valeuradhesion = 1.5;
                        break;
                    case 4:
                        $forfait = "FORFAIT INTERNET MTN : 2 SSI (07jrs)";
                        ...
                        break;
                    case 5:
                        $forfait = "FORFAIT INTERNET MTN : 3 SSI (07jrs)";
                        $valeuradhesion = 3;
                        break;
                    case 6:
                        $forfait = "FORFAIT INTERNET MTN : 4 SSI (07jrs)";
                        ...
                        break;
                    case 8:
                        $forfait = "FORFAIT INTERNET MTN : 6 SSI (30jrs)";
                        $valeuradhesion = 6;
                        break;
                    case 9:
                        $forfait = "FORFAIT INTERNET MTN : 10 SSI (30jrs)";
                        $valeuradhesion = 10;
                        break;
                    case 10:
                        $forfait = "FORFAIT INTERNET MTN : 20 SSI (30jrs)";
                        $valeuradhesion = 20;
                        break;
                    default:
                        $forfait = "inconnue";
            ...
            if (request('lib') == "depot") {
                ...
                {
                    flash("Vous devez être connecté pour effectuée le dépôt et payer les frais à partir de vos compte avoirs. 
                    Si vous ne possédez pas de compte, inscrivez-vous et revenez sur CELTIIS")->error();
                    return redirect('/connexion');
                }
            ...
            if (request('lib') == "forfait") {
                ...
                switch (request('forfaitlib')) {
                    ...
                    case 10:
                        ...
                        break;
                    case 11:
                        $forfait = "FORFAIT INTERNET MOOV : 30 SSI (30jrs)";
                        $valeuradhesion = 30;
                        break;
                    case 20:
                        $forfait = "FORFAIT INTERNET MOOV : 31 SSI (illimité)";
                        $valeuradhesion = 31;
                        break;
                    case 12:
            ...
            if (request('lib') == "depot") {
                ...
                if (!auth()->guest()) {
                    ...
                    if(isset(DB::table('avoirs')->where('id_user', auth()->user()->id)->where('gainvirtuel', '>=', $valeuradhesion)->get()[0]->gainvirtuel))
                    {
                        ...
                        //flash("Vous recevez un message de confirmation dans un instant.");
                        $message = "Vous ".auth()->user()->nom." (".auth()->user()->email.") avez payé "
                        .$valmotant." SSI pour ".$forfait." sur le numéro ".request('numero').". Frais SSI : ".$fraisajouter." SSI soit 0.5% du montant";
                        HistoriqueClient::saveHistorique($message, auth()->user()->id );
                        flash($message);
