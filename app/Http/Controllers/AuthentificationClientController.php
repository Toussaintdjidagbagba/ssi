...
class AuthentificationClientController extends Controller
{
        ...
        if ($resultat) {
            ...
            else
            {
                ...
                if ($pack == 1) { // 1 pour Pack Visiteur 0 $ SSI
                    $mes = " Code de validation : ".auth()->user()->Approbation;
                    $mailfilleul = auth()->user()->email;
                    InterfaceServiceProvider::EnvoieMail($mailfilleul, $mes, "Validation de compte sur SSI ".auth()->user()->Approbation, "Utiliser le code renseigné pour valider votre inscription");
                    flash("Utiliser le code suivant pour valider votre inscription. ".$mes);
                }else{
    ...
    public function saveinscription(Request $request)
    {
        ...
        if ($exit != "0" && request('cocherparrain')) {
            flash("Code du parrain incorrect!  Veuillez entrer soit le code du parrain ou choisissez 'Pas de code de parrainage'.")->error();
            return Back();
        }
        ...
        if ($exit != "0" || request('cocherparrain')) {
            ...
            if (!isset(DB::table('users')->where('email', request('mail'))->where('nomuser', $temp_pseudo)->get()[0]->nomuser)) {
                ...
                if (!isset(DB::table('users')->where('nom', request('nom'))->where('prenom', request('prenom'))->get()[0]->nomuser)) {
                    ...
                    $create = User::create([
                        ...
                        'numidentite' => request('numpie'),
                        'namepiece' => request('ide')
                    ]); 
                    
                    if ($request->hasFile('photopiece')) {

    		            $namefile = 'I_'.request('nom').'_'.request('prenom').'_'.$code_id_unique;
    		            $extavatar  = $request->file('photopiece')->getClientOriginalExtension();
    		            if($extavatar == "png" || $extavatar == "jpg" || $extavatar == "jpeg")
    		            {
        		            $upload = "photo/identite/";
        		            $request->file('photopiece')->move($upload, $namefile.'.'.$extavatar);
                	...
                	if ($request->hasFile('photo')) {
    		            ...
    		            $namefile = 'P_'.request('nom').'_'.request('prenom').'_'.$code_id_unique;
    		            $extavatar  = $request->file('photo')->getClientOriginalExtension();
    		            if($extavatar == "png" || $extavatar == "jpg" || $extavatar == "jpeg")
    		            {
        		            $upload = "photo/profil/";
        		            $request->file('photo')->move($upload, $namefile.'.'.$extavatar);
                ...
                {
                    flash("Le nom et prénom existe déjà pour ce mail... Veuillez vous connecter ou cliquer sur mot de passe oublié si vous ne sourvenez pas.")->error();
                    return Back();
            ...
            if ($pack == 1) { // 1 pour Pack Visiteur 0 $ SSI
                $mes = " Code de validation : ".$codeapprobation;
                $mailfilleul = request('mail');
                //InterfaceServiceProvider::EnvoieMail($mailfilleul, $mes, "Validation de compte sur SSI ".$codeapprobation, "Utiliser le code renseigné pour valider votre inscription");
                flash("Utiliser le code suivant pour valider votre inscription. ".$mes);
            }else{
